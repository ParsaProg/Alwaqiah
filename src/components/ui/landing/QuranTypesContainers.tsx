...
export default function QuranContainerTypes() {
  ...
  return (
    <div className="mt-[100px] [@media(min-width:1495px)]:w-[90%] w-[90%] p-8 flex-wrap bg-primary-Light rounded-xl flex items-center justify-center md:gap-[80px] gap-10 mx-auto">
      {items.map((item, _i) => {
        return <div key={_i} className="text-white flex  flex-col items-center gap-y-1">
            ...
            <h2 className="font-thin text-xs">{item.subTitle}</h2>
        </div>
      })}
    </div>
  );
